...
import { Sun, Moon, Star, ArrowUp, Compass, Sparkles } from 'lucide-react'
...
export function NatalChartResult({ chartData, userName }: NatalChartResultProps) {
  if (!chartData) return null

  const { ascendant, mc, planets, houses, birthDate, birthTime, location } = chartData

  // Sécurité pour trouver le Soleil (français ou anglais)
  ...
  return (
    <div className="space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-1000 max-w-6xl mx-auto pb-20">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
        ...
        {/* Signe Solaire */}
        <div className="group relative bg-black/40 border border-white/5 rounded-none p-10 text-center transition-all duration-500 hover:border-amber-500/40">
        ...
        {/* Ascendant */}
        <div className="group relative bg-black/40 border border-white/5 rounded-none p-10 text-center transition-all duration-500 hover:border-purple-500/40">
        ...
        {/* MC */}
        <div className="group relative bg-black/40 border border-white/5 rounded-none p-10 text-center transition-all duration-500 hover:border-blue-500/40">
      ...
      {/* Planètes et Maisons (Style minimaliste) */}
          ...
          <div className="grid grid-cols-2 gap-x-8 gap-y-4">
            {houses?.slice(0, 12).map((h) => (
              <div key={h.number} className="flex justify-between items-center text-[10px] tracking-widest border-b border-white/5 pb-2">
                ...
                <span className="text-white/80">{h.sign}</span>
              </div>
            ))}
          </div>
        </div>
